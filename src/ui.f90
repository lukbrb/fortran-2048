module ui
    ...
    !       _rl désigne une taille relative
    ...
    type(color_type),   parameter :: CLR_SUPERTILE = color_type(128, 0, 128, 255)  ! Pour toutes les cases supérieures à 2048
    
    ! Lien automatique entre le numéro et la couleur  
    integer :: l
    ...
    ! Couleurs de la grille et des boutons
    ...
    ! Bouton permanent pour relancer une partie
    type(Button_Style), parameter :: new_button_style = Button_Style( &
                                                color = restart_button_color, &
                                                hover = -0.20, &
                                                hold = -0.30)
    ! Bouton pour recommencer lorsque la partie est perdue
...
contains
! --------------------------------------------------------------------------------------------------------------------------
! -------------------------------------- Procédures pour afficher les cases et boutons -------------------------------------
! --------------------------------------------------------------------------------------------------------------------------

    subroutine render_board(board_x_px, board_y_px, board_size_px, board, restart_game)
        ! TODO: Décompléxifier cette fonction, notament déplacer le bouton
        ...
        ! Note: affecter directement la valeur de new_game_button_clicked à restart_game ?
    ...
    ! ---------------------------------- Procédures pour la gestion des boutons ----------------------------------
    ...
    function new_game_button_clicked(board_x_px, board_y_px, board_size_px) result(clicked)
      ...
      function button_logic(id, boundary, state) result(clicked)
        ! Note: Fonction directement empruntée à Tsoding : https://github.com/tsoding/tic-tac-toe-fortran-raylib/blob/master/src/ui.f03
        integer,         intent(in)  :: id
...
! ------------------------ Gestion des couleurs et de l'affichage ------------------------------
    ...
    subroutine display_game_over(restart_game)
        ! Affiche un fond d'écran sombre et transparent, ainsi qu'un bouton pour relancer le jeu.
        type(Rectangle) :: rec
        type(Button_type) :: button
        logical, intent(inout) :: restart_game
    ...
    ! Affiche un message sur un fond d'écran sombre transparent selon la même modèle que la procédure du dessus. 
    ! TODO: Néanmoins pas de fonction "sleep" (extension GNU), donc le message n'apparaît que brièvement
